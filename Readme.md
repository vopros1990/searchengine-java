...
- [Конфигурация](#конфигурация)
    ...
    - [Переключение между реализациями поискового запроса](#переключение-между-реализациями-поискового-запроса)
...
### Переключение между реализациями поискового запроса

```yaml
searching:
  mode: jpql # sql / jpql - default
```
...
### Поисковый запрос
Доступно две реализации поиска - через Native SQL и JPQL.
Коротко скажу лишь о том, что испытываю гордость и удовлетворение за
то, что вся поисковая выдача SQL реализации формируется лишь одним сложным нативным SQL запросом, что сильно экономит ресурсы памяти.
